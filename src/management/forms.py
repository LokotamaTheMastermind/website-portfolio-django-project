...
class ProjectForm(forms.ModelForm):
    ...
    project_picture = forms.ImageField(
        widget=forms.FileInput(attrs={'class': 'custom-file-input'}), label='Project Picture', allow_empty_file=True)
    ...
    extra_information = forms.FileField(widget=forms.FileInput(
        attrs={'class': 'custom-file-input'}), label='More Information File')
    is_in_progess = forms.BooleanField(label='Done')
